...
import random
def getPlebaniData(path,plebaniMICAclass,targetWavelengthSet,perLabelData=100,MICA=True):
    trdr_wavelengths = [
    ...
    if MICA:
        targetWavelengthSet=targetWavelengthSet[len([w for w in targetWavelengthSet if w<trdr_WavelengthSet[0]]):len(targetWavelengthSet)-len([w for w in targetWavelengthSet if w>trdr_WavelengthSet[-1]])]
    ...
    for l in label_dict:
        ...
        if len(label_dict[l])>=perLabelData*1.2:
            count=0
            for i in random.sample(list(np.arange(len(label_dict[l]))),int(perLabelData*1.2)):
    ...
    if MICA: return X,Y,targetWavelengthSet
    else: return X,Y